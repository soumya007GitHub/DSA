class Solution {
    ...
    public int helper(int index, int buyStatus, int[] prices, int[][] dp){
        ...
        if(buyStatus == 0){
            ...
            int buy = -prices[index] + helper(index+1, 1, prices, dp);
            // skip current stock
            int notBuy = helper(index+1, 0, prices, dp);
            ans = Math.max(buy, notBuy);
        }
        // purchased any other stock previously
        else{
            // sell stock
            int sell = prices[index] + helper(index+1, 0, prices, dp);
            // dont sell stock
            int notSell = helper(index+1, 1, prices, dp);
